...
os.environ['HOMESTAY_DB_PATH'] = str(_TMPDIR / 'test_db_retrieval_tmp.sqlite')
sys.path.insert(0, '.')
...
# (retrieve giờ LUÔN kèm mẩu pinned lên đầu → Phòng 301 là mẩu match ngay sau)
h = kb.retrieve("phong 301\nbao nhieu 1 dem", SHOP)
check(h and any(c["title"] == "Phòng 301" for c in h[:2]), "C2 followup_context", titles(h))
...
# Kho lớn (vượt ngưỡng 24k) → mode 'retrieval', chỉ top-k (+pinned nếu có)
big = [{"title": f"Mẩu {i}", "content": "x" * 700 + f" so {i}",
        "keywords": [f"so {i}"]} for i in range(40)]
kb.ingest(big, "big-shop")
ch, mode = kb.context_chunks("so 7", "big-shop")
check(mode == "retrieval" and len(ch) <= 7, "D3 big_kb_retrieval", f"mode={mode} n={len(ch)}")
