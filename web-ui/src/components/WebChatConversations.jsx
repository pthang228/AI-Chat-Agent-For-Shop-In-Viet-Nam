...
export default function WebChatConversations() {
  ...
  async function onSetOwner(uid) {
    if (!confirm(t("cv.set_owner_confirm_site"))) return;
    const r = await webchat.setOwner(uid);
    alert(r.ok ? t("cv.set_owner_ok_site") : t("cv.set_owner_fail"));
  }
  ...
  if (offline)
    return (
      <div className="connect">
        <div className="status warn">{t("cv.offline", { name: "Webchat", port: 5011 })}</div>
        <p className="hint">{t("cv.run_pre")} <code>python -m app.main_webchat</code> {t("cv.run_post")}</p>
      </div>
    );
    ...
    <div>
      {sites.length > 1 && (
        <div className="page-tabs">
          <button className={"page-tab" + (siteId === "" ? " active" : "")} onClick={() => setSiteId("")}>{t("cv.all")}</button>
          {sites.map((s) => (
        ...
        <div className="chatview">
          <div className="chat-top">
            ...
            <div className="chat-actions">
              <button className="btn-mini" onClick={() => onSetOwner(detail.user_id)} title={t("cv.set_owner_title_site")}>{t("cv.set_owner_btn")}</button>
