...
export default function ShopeeConversations() {
  ...
  async function onSetOwner(uid) {
    if (!confirm("Đặt người này làm CHỦ SHOP (nhận tin nhắn báo khi khách cần)?")) return;
    const r = await shopee.setOwner(uid);
    alert(r.ok ? "✅ Đã đặt làm chủ shop." : "❌ Lỗi đặt chủ.");
  }
  ...
  if (offline)
    return (
      <div className="connect">
        ...
        <p className="hint">Chạy <code>python -m app.main_shopee</code> rồi tải lại.</p>
      </div>
    );
    ...
    <div>
      {shops.length > 1 && (
        <div className="page-tabs">
          <button className={"page-tab" + (shopId === "" ? " active" : "")} onClick={() => setShopId("")}>Tất cả</button>
          {shops.map((s) => (
        ...
        <div className="chatview">
          <div className="chat-top">
            ...
            <div className="chat-actions">
              <button className="btn-mini" onClick={() => onSetOwner(detail.user_id)} title="Đặt người này làm chủ shop">⭐ Đặt làm chủ</button>
