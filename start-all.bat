...
echo [7/9] Zalo OA bot ...
start "Zalo OA Bot" /D "%~dp0" cmd /k python -m app.main_zalo_oa

echo [8/9] Webchat bot (widget website khach hang) ...
start "Webchat Bot" /D "%~dp0" cmd /k python -m app.main_webchat

echo [9/9] Web UI (giao dien quan ly - HTTPS de test Facebook Login) ...
start "Web UI" /D "%~dp0web-ui" cmd /k npm run dev:https
